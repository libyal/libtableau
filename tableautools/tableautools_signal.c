...
void (*tableautools_signal_signal_handler)( tableautools_signal_t ) = NULL;

/* Signal handler for Ctrl+C or Ctrl+Break signals
 */
BOOL WINAPI tableautools_signal_handler(
             tableautools_signal_t signal )
{
	static char *function = "tableautools_signal_handler";
	...
	switch( signal )
	{
		...
		case CTRL_C_EVENT:
			if( tableautools_signal_signal_handler != NULL )
			{
				tableautools_signal_signal_handler(
				 signal );
			}
			return( TRUE );
 ...
 */
void tableautools_signal_initialize_memory_debug(
      void )
{
	int flag = 0;
 ...
 */
int tableautools_signal_attach(
     void (*signal_handler)( tableautools_signal_t ),
     libcerror_error_t **error )
{
	static char *function = "tableautools_signal_attach";
	...
	tableautools_signal_signal_handler = signal_handler;

	if( SetConsoleCtrlHandler(
	     (PHANDLER_ROUTINE) tableautools_signal_handler,
	     TRUE ) == 0 )
	{
		libcerror_error_set(
		 error,
...
#if defined( _MSC_VER )
	tableautools_signal_initialize_memory_debug();
#endif
 ...
 * Returns 1 if successful or -1 on error
 */
int tableautools_signal_detach(
     libcerror_error_t **error )
{
	static char *function = "tableautools_signal_detach";
	...
	tableautools_signal_signal_handler = NULL;
