...
LIBRARY_TESTS="error notify security_values support values_table";
...
run_test()
{
	...
	local TEST_EXECUTABLE="./tableau_test_${TEST_NAME}";
...
run_test_with_input()
{
	...
	run_test_on_input_directory "libtableau" "${TEST_DESCRIPTION}" "default" "${OPTION_SETS}" "${TEST_EXECUTABLE}" "input" "${INPUT_GLOB}";
	local RESULT=$?;
